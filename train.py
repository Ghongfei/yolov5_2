...
def train(hyp):
    ...
    # Load Model 加载预训练模型和权重，并写入训练结果到results.txt
    ...
    # Mixed precision training https://github.com/NVIDIA/apex 把混合精度训练加载入训练中
    if mixed_precision:
        model, optimizer = amp.initialize(model, optimizer, opt_level='O1', verbosity=0)

    # Scheduler https://arxiv.org/pdf/1812.01187.pdf 设置cosine调度器，定义学习率衰减
    ...
    # Initialize distributed training 定义并初始化分布式训练
    ...
    # Start training 获取参数 获取开始时间，batch size数量，epochs数量，图片数量
    t0 = time.time()
    ...
    '''
    载图片权重（可选），定义进度条，设置偏差Burn-in，使用多尺度，前向传播，
    损失函数，反向传播，优化器，打印进度条，保存训练参数至tensorboard，
    计算mAP，保存结果到results.txt，保存模型（最好和最后）
    '''
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            results, maps, times = test.test(opt.data,
                                             batch_size=batch_size,
                                             imgsz=imgsz_test,
                                             save_json=final_epoch and opt.data.endswith(os.sep + 'smoke.yaml'),
                                             model=ema.ema,
                                             single_cls=opt.single_cls,
...
if __name__ == '__main__':
    ...
    # Evolve hyperparameters (optional) 根据训练结果进行超参数的进化
    else:
        tb_writer = None
